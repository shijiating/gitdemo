...
def bp_train(feature, label, n_hidden, maxCycle, alpha, n_output):
    ...
    input:  feature(mat):特征
            label(mat):标签
            n_hidden(int):隐含层的节点个数  20
            maxCycle(int):最大的迭代次数   5000
            alpha(float):学习率   0.008
            n_output(int):输出层的节点个数  1
...
if __name__ == "__main__":
    ...
    # 1、导入数据
    print("--------- 1.load data ------------")
    feature, label, n_output = load_data("data_train_500.txt")
    print("---------feature------------", feature)
    print("---------label------------", label)
    print("---------n_output------------", n_output)
    # 2、训练网络模型
    print("--------- 2.training ------------")
    center, delta, w = bp_train(feature, label, 9, 500, 0.005, n_output)
    # 3、得到最终的预测结果
    print("--------- 3.get prediction ------------")
    result = get_predict(feature, center, delta, w)
    print("训练准确率为：", (1 - err_rate(label, result)))
    # 4、保存最终的模型
